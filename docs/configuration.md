...
Backlog is passive. Todo is the entry point autonomous runners claim. Tickets also carry `kind` (`issue`, `spec`, `ticket`, `decision`, `map`), `source` (`manual`, `triage`, `to_spec`, `to_tickets`, `wayfinder`, `donna`), an optional category, plus title, description, acceptance criteria, status, priority, type, tags, dependencies, optional runner assignment, role instructions, environment profile, and comments.
...
| `automation.autoMerge` | `false` | Compatibility field; final merges always require the human Review-card button |
| `automation.autoPush` | `true` | Push ticket delivery branches and user-approved integration results |
| `automation.actionableStatuses` | Todo, In Progress, QA | States that wake runners |
...
Development, QA, and repair loops run automatically. A QA-passed issue is sealed to its own delivery branch and moved to Review. Only the human **Merge to `<integrationBranch>`** button integrates it; after a successful merge, the delivery is marked merged and dependent tickets become eligible. No automatic merge or Done status exists.

Issue details are editable in Backlog and Blocked. Todo, In Progress, QA, and Review are read-only while agents own them. Active-stage drawers expose an emergency **Abort process** button that interrupts and unassigns active runners, clears the explicit assignment, and moves the issue to Blocked for human instructions.

When a runner genuinely needs a decision, it must record the exact question and a safe recommended answer. The Blocked drawer shows both and the response deadline. If nobody answers before `automation.humanInputTimeoutMinutes`, the heartbeat records the recommendation as decision input, moves the issue back to Todo, and resumes delivery. Emergency user-aborted issues never auto-resume.
