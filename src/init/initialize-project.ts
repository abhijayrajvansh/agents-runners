...
When \`.codex-runners/config.json\` exists, use the Codex Runners MCP tools as the shared task source. Donna is the persistent project manager. Backlog is inactive; actionable tickets are processed autonomously. Do not create unmanaged worker processes or remove persistent runner worktrees. Never print environment-file values into chat, logs, tickets, or commits.`;
