...
async function createRepository(branch = "dev") {
  const root = await mkdtemp(path.join(tmpdir(), "codex-runners-init-"));
...
describe("initializeProject", () => {
  it("creates project config while preserving existing rules and hooks", async () => {
    ...
    const pluginRoot = "/Users/example/plugins/codex-runners";
    ...
    const first = await initializeProject(root, { pluginRoot, nodePath: "/usr/bin/node" });
    const second = await initializeProject(root, { pluginRoot, nodePath: "/usr/bin/node" });
    const config = JSON.parse(await readFile(path.join(root, ".codex-runners", "config.json"), "utf8"));
    const agents = await readFile(path.join(root, "AGENTS.md"), "utf8");
    const hooks = JSON.parse(await readFile(path.join(root, ".codex", "hooks.json"), "utf8"));
    const ignore = await readFile(path.join(root, ".gitignore"), "utf8");

    expect(first.created).toContain(".codex-runners/config.json");
    ...
    expect(agents.match(/codex-runners:start/g)).toHaveLength(1);
    expect(hooks.hooks.Stop[0].hooks[0].command).toBe("notify-stop");
    expect(hooks.hooks.SessionStart).toHaveLength(1);
    expect(ignore.match(/\.codex-runners\/runtime\//g)).toHaveLength(1);
  });
